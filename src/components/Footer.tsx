import { Phone, MapPin, TreeDeciduous, Home } from "lucide-react";
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
type FooterLabels = {
  ...
  companyBlurb: string;
  serviceAreas: string;
  serviceAreasDetail: string;
  contactUs: string;
  spanish: string;
  english: string;
  whatsappButton: string;
  tooltipCall: string; // used as prefix with name
  tooltipMessage: string;
  designedWithRGV: string;
  allRights: string;
};
...
const defaultLabels: FooterLabels = {
  ...
  companyBlurb:
    "Providing professional tree and palm trimming services with 24 years of experience. We take pride in enhancing the health, safety, and beauty of your trees and outdoor spaces.",
  ...
  tooltipCall: "Call or text",
  tooltipMessage: "Message us on WhatsApp",
  designedWithRGV: "Designed with ❤️ in the RGV",
...
const QuickLinks = ({ labels }: { labels: FooterLabels }) => {
  const links = [
    { name: labels.services, href: "#services", Icon: TreeDeciduous },
    { name: labels.contact, href: "#contact", Icon: Phone },
    { name: labels.getEstimate, href: "#contact", Icon: Home },
  ];
          ...
          <li key={link.name}>
            ...
            >
              <div className="h-7 w-7 rounded-md bg-green-50/80 border border-green-100 flex items-center justify-center flex-shrink-0 group-hover:bg-green-100 transition-colors">
                <link.Icon className="h-3.5 w-3.5 text-green-600 group-hover:scale-110 transition-transform duration-200" />
              </div>
...
const Footer = ({ labels }: FooterProps) => {
  ...
  const l: FooterLabels = { ...defaultLabels, ...(labels || {}) };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="py-12 md:py-16 grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-8">
          ...
          <div>
            ...
            >
              ...
              <span className="hidden sm:inline font-serif tracking-wide ml-1.5 drop-shadow-md group-hover:drop-shadow-lg transition-all duration-200 bg-gradient-to-b from-green-500 to-green-950 text-transparent bg-clip-text">
                Rodriguez Trimming Services
              </span>
            </a>
            ...
            <TooltipProvider>
              ...
              <div className="space-y-4 mb-8">
                ...
                <div className="flex items-center gap-3 group">
                  ...
                  <div>
                    <p className="text-xs text-gray-500">{l.spanish}</p>
                    <Tooltip>
                ...
                <div className="flex items-center gap-3 group">
                  ...
                  <div>
                    <p className="text-xs text-gray-500">{l.english}</p>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <a
              ...
              <div className="flex justify-center md:justify-start">
                <Tooltip>
                  <TooltipTrigger asChild>
                    ...
                    >
                      <WhatsAppIcon className="h-5 w-5 active:scale-97 transition-transform ease-out" />
        ...
        <div className="border-t border-gray-200/80 py-5 flex flex-col md:flex-row justify-between items-center">
          <p className="text-gray-500 text-sm order-2 md:order-1 mt-3 md:mt-0">
            &copy; {currentYear} Rodriguez Tree & Palm Trimming Service.{" "}
            {l.allRights}
          </p>

          <div className="flex items-center gap-2 order-1 md:order-2">
            <div className="flex items-center text-xs text-gray-500">
              <span>{l.designedWithRGV}</span>
            </div>
          </div>
        </div>
      </div>
